      ...
      <div className="max-w-5xl mx-auto">
        ...
        >
          {PARTNERS.map(({ name, role, body, href, cta, Icon }) => (
            <div
              key={name}
              className="bg-purple-900/30 border border-purple-500/30 backdrop-blur-sm rounded-lg p-7 max-w-md w-full hover:border-cyan-400/60 hover:bg-purple-900/40 transition"
            >
              ...
              <p className="mt-3 text-purple-200 leading-relaxed">{body}</p>

              <a
                href={href}
                target="_blank"
                rel="noopener noreferrer"
                className="mt-5 inline-flex items-center gap-1.5 text-sm font-semibold text-cyan-300 hover:text-cyan-200 transition"
              >
